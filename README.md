# 👋 Hola, soy Tomas Romagnoli

🎓 Estudiante de 4º año de **Ingeniería en Informática** en la Universidad Nacional Arturo Jauretche (UNAJ).  
💻 Desarrollador **Backend / Fullstack Junior** con experiencia en **C#, .NET, SQL Server, React y Docker**.  
🚀 Apasionado por construir aplicaciones robustas, seguras y escalables, aplicando buenas prácticas como **Clean Architecture** y principios **SOLID**.  

---

## 🔧 Tecnologías y Herramientas

- **Lenguajes:** C#, TypeScript, JavaScript, SQL, Python  
- **Backend:** ASP.NET Core, Entity Framework Core, REST APIs  
- **Frontend:** React, HTML5, CSS3, Tailwind CSS  
- **Bases de datos:** SQL Server, MySQL  
- **Testing & QA:** xUnit, NUnit, Postman, Swagger/OpenAPI  
- **DevOps & Cloud:** Docker, Git/GitHub, AWS  
- **Arquitectura:** MVC, Clean Architecture, Microservicios, Patrones de diseño  

---

## 📌 Proyectos Destacados

### 🎟️ Plataforma de venta de entradas
- Backend: ASP.NET Core, Clean Architecture, Microservicios  
- Frontend: React + TypeScript
- Logro: Visualización dinámica de espacios y gestión de tickets en tiempo real  
- 🔗 [Repositorio en GitHub](https://github.com/PS-Grupo-3)

---

### 🛒 Sistema de gestión para supermercado
- ASP.NET Core
- CRUD de productos, stock y ventas  
- 🔗 [Repositorio en GitHub](https://github.com/Tomiroma/MiniMercado)

---

### ☕ Sitio web para cafetería
- JavaScript + HTML + CSS  
- Diseño responsive y modular  
- 🔗 [Repositorio en GitHub](https://github.com/Tomiroma/Proyecto-Ing-de-soft-I-Starbucks)

---

## 🌍 Idiomas
- Español: Nativo  
- Inglés: B2 (Cambridge First Certificate)  

---

## 📫 Contacto
- 📧 Email: **romagnolitomas2003@gmail.com**  
- 🔗 LinkedIn: https://www.linkedin.com/in/tomas-romagnoli-622823232/
- 💻 GitHub: [github.com/Tomiroma](https://github.com/Tomiroma)  

---
✨ *Siempre aprendiendo, siempre construyendo.*
